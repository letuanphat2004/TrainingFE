...
import type { Content, CssVariableStyle, SocialLink, TeamMemberLayout, TeamMemberTypography } from "../../../types/ui";
...
function toLayoutVariables(layout: TeamMemberLayout): CssVariableStyle {
    return Object.entries(layout).reduce((variables, [name, value]) => {
        const variableName = layoutVariableNames[name as keyof typeof layoutVariableNames];
...
function toTypographyVariables(typography: TeamMemberTypography): CssVariableStyle {
    return Object.entries(typography).reduce((variables, [name, value]) => {
        const variableName = typographyVariableNames[name as keyof typeof typographyVariableNames];
        if (variableName && value !== undefined) {
...
interface TeamMemberCardProps {
  ...
  style?: CSSProperties;
  typography?: TeamMemberTypography;
}

export function TeamMemberCard({ className = "", description, image, isFeatured = false, layout = {}, name, role, socialLinks, style, typography = {} }: TeamMemberCardProps) {
    const classes = [
        "team-member-card",
