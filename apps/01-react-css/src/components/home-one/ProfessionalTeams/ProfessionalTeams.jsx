...
const baseMemberLayout = Object.freeze({
    cardWidth: 270,
    cardHeight: 439,
    imageTop: 0,
    imageLeft: 62,
...
export function ProfessionalTeams() {
    return (<section className="professional-teams" id="team" aria-labelledby="professional-team-title">
      ...
      {teamMembers.map((member) => (<TeamMemberCard className="professional-teams__member" description={memberDescription} image={member.image} isFeatured={member.isFeatured} key={member.id} layout={member.layout} name={member.name} role={member.role} socialLinks={socialLinks} style={{ "--team-member-left": `${member.localLeft}px` }}/>))}
    </section>);
}
