    ...
    return (<section className="core-services page-component" id="services" aria-labelledby="services-title">
      ...
      <div className="core-services__cards">
        {services.map((service) => {
          const { left, ...serviceProps } = service;
          const style: CssVariableStyle = { "--service-card-left": `${left}px` };

          return <ServiceCard className={`core-services__card core-services__card--${service.id}`} key={service.id} style={style} {...serviceProps} />;
        })}
      </div>
    </section>);
}
